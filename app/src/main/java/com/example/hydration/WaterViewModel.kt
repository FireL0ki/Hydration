...
class WaterViewModel(private val repository: WaterRepository): ViewModel() {
    ...
    fun getRecordForDay(day: String): LiveData<WaterRecord> {
        return repository.getRecordForDay(day).asLiveData()  // flow can watch the database, notify of db changes
    }

}

// T is a generic class placeholder for the class definition
